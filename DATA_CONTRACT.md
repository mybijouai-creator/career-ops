...
| `_accounts.db` (repo root, sibling of `cv.md` — not under `data/`) | Unlike every other row in this table, this is not *your* data — it's the multi-tenant accounts store: every signed-up user's email, password hash (scrypt), and their own encrypted provider API key (AES-256-GCM, keyed by `CAREER_OPS_ENCRYPTION_KEY`), plus session tokens. Absent entirely on a single-tenant install with no accounts. `node:sqlite` (Node 22.5+ built-in — no native-module dependency); WAL mode, so `-wal`/`-shm` sidecar files sit beside it and are equally sensitive. Never committed, never touched by an update. Schema and queries: `web/src/lib/auth/db.mjs`; session cookie handling: `web/src/lib/auth/session.mjs`. |
