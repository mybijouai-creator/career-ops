...
| `data/push-subscriptions.json` | Your Web Push subscriptions, one per browser that opted in (`endpoint`, `keys`, `addedAt`, `ua`). Written by the web app when you enable notifications; the endpoint is the identity, so re-subscribing a device updates its row rather than appending a duplicate. Dead endpoints (HTTP 404/410/403/401 from the push service) are pruned on the next send. Delete the file to unsubscribe every device. |
| `config/vapid.json` | Your Web Push signing keypair, generated once on first use and mode 0600. Regenerating it silently invalidates every existing subscription, because the push service rejects a payload signed by a key that does not match the one the subscription was created with — so this file must persist alongside your other data. Overridden by `CAREER_OPS_VAPID_PUBLIC_KEY` / `CAREER_OPS_VAPID_PRIVATE_KEY` when both are set, which is how a deployment keeps the private key out of the data volume. It authorizes pushing to your own subscriptions and nothing else; it is not an account credential. |
| `data/worker-state.json` | When each background worker last ran, and what has been spent today (`{workers: {id: {lastRunAt, lastOk, lastNote, lastMs, running, enabled}}, spend: {day, usd}}`). Written by the web app's scheduler. Persisted so a restart does not re-run every worker immediately — without it a redeploy would scan, recheck liveness and start evaluating on boot. The `spend` bucket is keyed by LOCAL calendar day and rolls over on read, so yesterday's spend never counts against today's ceiling (`CAREER_OPS_DAILY_USD_CAP`, default $2.25). A `running` flag left by a crashed process is cleared at scheduler start. |
| `data/gates.tsv` | Your append-only approval-gate ledger: `{gate_id}\t{at}\t{run_id}\t{kind}\t{decision}\t{actor}\t{est_cost_usd}\t{will_write}\t{note}`. Written by the web app whenever a run reaches a write-or-send boundary and stops to ask you (HANDOFF.md §5). Never edited in place — a decision appends a SECOND row beside the `pending` one, so the whole lifecycle is recorded and an approval cannot be quietly changed after the fact. `decision` is one of `pending\|approved\|rejected\|expired`; `will_write` is a comma-joined path list (a TSV cannot nest); an absent value is the `-` sentinel, never an empty cell. `actor` is `local` while the app has no authentication — there is no identity to record, and writing a name it cannot know would be worse than the honest placeholder. Gates expire unapproved after 24h. Column spec and parser: `web/src/lib/gates/core.mjs` |
| `runs/*.jsonl` | Your per-run event ledger, one file per run: newline-delimited `plan`/`step`/`artifact`/`gate`/`usage`/`done`/`error` events (HANDOFF.md §4). Appended by the web app's runner; the audit trail for what a run did, what it cost and which files it touched. Never edited in place. Event contract: `web/src/lib/runs/events.mjs` |
| `_accounts.db` (repo root, sibling of `cv.md` — not under `data/`) | Unlike every other row in this table, this is not *your* data — it's the multi-tenant accounts store: every signed-up user's email, password hash (scrypt), and their own encrypted provider API key (AES-256-GCM, keyed by `CAREER_OPS_ENCRYPTION_KEY`), plus session tokens and two Phase-4 rate-limit tables (`login_failures`, `signup_attempts` — persisted here rather than an in-process `Map` specifically so a deploy/restart can't reset an attacker's throttle). Absent entirely on a single-tenant install with no accounts. `node:sqlite` (Node 22.5+ built-in — no native-module dependency); WAL mode, so `-wal`/`-shm` sidecar files sit beside it and are equally sensitive. Never committed, never touched by an update. Schema and queries: `web/src/lib/auth/db.mjs`; session cookie handling: `web/src/lib/auth/session.mjs`. |
| `tenants/{userId}/` (repo root, sibling of `cv.md` — not under `data/`) | Where a signed-up user's OWN copy of every other row in this table actually lives, once they have an account: `tenants/{userId}/cv.md`, `tenants/{userId}/config/profile.yml`, `tenants/{userId}/data/applications.md`, and so on — one full, independently seeded tree per user, keyed by their 128-bit random id (never their email, never guessable). Every web request from a signed-in user is transparently scoped to their own tree (`web/src/lib/auth/tenant-context.mjs`, an `AsyncLocalStorage`-carried root — see `web/src/lib/auth/with-tenant.mjs`); a request with no valid session falls through to the original single-tenant shared root at the repo root exactly as if this row didn't exist, so a single-tenant install with zero signups is unaffected. Seeded on first use by reusing the same script the container's own boot-time seed runs (`web/docker-seed-root.sh`), not a second reimplementation. **Known gap:** the background workers (`/api/workers`, scan/batch-eval/liveness) run on a process-wide timer outside any request's tenant context, so they only ever act on the single shared/default root — not yet per-tenant. |
| `cvs/index.json` + `cvs/{id}.md` (tenant root, sibling of `cv.md`) | Web-only: a tenant's named base CVs (e.g. "Backend", "AI/ML"), layered ON TOP of the single `cv.md` every mode/script in the whole system still reads directly — see `web/src/lib/cv-library.mjs`'s header for why it's a layer rather than a rework of every consumer. `index.json` is `{activeId, cvs: [{id, name, createdAt, updatedAt}]}`; each CV's content lives in its own `{id}.md`. Whichever CV is `activeId` is kept mirrored into `cv.md` on every write that touches it (create/update/activate), so the invariant "cv.md == the active named CV's content" always holds and nothing outside the web app needs to know this feature exists. Absent entirely until a tenant creates their first named CV — a legacy account with only `cv.md` is migrated into a single "Default" entry the first time it's read, non-destructively and only once. `id`s are generated (slugified from the name, `-2`/`-3` suffixed on collision) and validated (`^[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$`) before ever reaching a path — never trust one from a URL param unchecked. |
| `data/cv-history.tsv` | Web-only, append-only: one row per tailored CV a web `pdf` run has actually rendered — `{date}\t{reportNum}\t{companySlug}\t{baseCvId\|-}\t{baseCvName\|-}\t{outputFile}`. Appended by `web/src/app/api/run/route.ts` right after the render is CONFIRMED on disk (never optimistically — same discipline as the tracker's PDF column). `baseCvId`/`baseCvName` are the `-` sentinel when the tenant has never used the named-CV feature above. Distinct from `/api/cv-pdf`'s fuzzy "newest PDF matching this company name" lookup: this is the exact, structured record, keyed by report number, of every render — the two can diverge once a company has more than one tailored CV on file. Never rewritten; a malformed hand-edited row is skipped and counted, not fatal. Read by `web/src/lib/cv-history.mjs`. |
